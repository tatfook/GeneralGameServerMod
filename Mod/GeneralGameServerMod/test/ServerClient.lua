...
local function AddPublicFiles()
    --NPL.AddPublicFile("ServerClient.lua", 1);
end
...
local function InitClient()
	...
	for i=1, nServerThreadCount do
		local rts_name = "worker"..i;
        rts_name = "main";
		--while( NPL.activate(string.format("(%s)simpleserver:ServerClient.lua", rts_name), 
		while( NPL.activate(string.format("(%s)simpleserver:test.lua", rts_name), 
